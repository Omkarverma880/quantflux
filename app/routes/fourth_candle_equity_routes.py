"""
API routes for the 4th-Candle CASH-EQUITY Strategy (Equity Strategy #3).

Backtest + simulate are read-only. Status/start/stop/positions drive the
paper/live strategy (trades the stock as MIS/CNC). Real orders only when
paper_trade is off AND the global trading gate is on. Mirrors the options
4th-candle route conventions.
"""
...
from research.fourth_candle_equity import FourthCandleEquityResearch
from research.fourth_candle_equity.config import load_config, save_config
from strategies.fourth_candle_equity_strategy import FourthCandleEquityStrategy

router = APIRouter()
logger = get_logger("api.fourth_candle_equity")

_STATE_FILE = settings.DATA_DIR / "strategy_configs" / "fourth_candle_equity_config.json"
_research: dict[int, FourthCandleEquityResearch] = {}
_strategies: dict[int, FourthCandleEquityStrategy] = {}
...
def _get_research(broker: Broker, user_id: int) -> FourthCandleEquityResearch:
    eng = _research.get(user_id)
    if eng is None:
        eng = FourthCandleEquityResearch(broker, user_id=user_id)
...
def _save_strategy_cfg(cfg: dict):
    try:
        ...
        logger.error("4th-candle-EQ strategy cfg save failed: %s", exc)


def _get_strategy(broker: Broker, user_id: int) -> FourthCandleEquityStrategy:
    strat = _strategies.get(user_id)
    if strat is None:
        strat = FourthCandleEquityStrategy(broker, _load_strategy_cfg(), user_id=user_id)
...
def backtest(payload: BacktestReq | None = None, user_id: int = Depends(login_required), db: Session = Depends(get_db)):
    ...
    try:
        ...
        logger.error("4th-candle-EQ backtest failed: %s", exc)
...
def simulate(payload: SimulateReq, user_id: int = Depends(login_required), db: Session = Depends(get_db)):
    ...
    try:
        ...
        logger.error("4th-candle-EQ simulate failed: %s", exc)
        return {"status": "error", "message": str(exc)}
