...
export const api = {
  ...
  hbTodayRefresh: (body) => request('/equity-strategy/hammer-breakout/today/refresh', { method: 'POST', body: JSON.stringify(body || {}) }),
  hbTodayAutoAlert: (body) => request('/equity-strategy/hammer-breakout/today/auto-alert', { method: 'POST', body: JSON.stringify(body || {}) }),
  ...
  oiLabHistory: (underlying = 'NIFTY') => request(`/index-strategy/oi-lab/history?underlying=${encodeURIComponent(underlying)}`),
  oiLabHistoryRebuild: (underlying = 'NIFTY') => request(`/index-strategy/oi-lab/history/rebuild?underlying=${encodeURIComponent(underlying)}`, { method: 'POST' }),
  oiLabDesk: (index) => request(`/index-strategy/oi-lab/desk?index=${encodeURIComponent(index)}`),
  oiLabSignalBacktest: (index) => request(`/index-strategy/oi-lab/signal-backtest?index=${encodeURIComponent(index)}`),
  oiLabSignalBacktestRun: (index) => request(`/index-strategy/oi-lab/signal-backtest/run?index=${encodeURIComponent(index)}`, { method: 'POST' }),
  oiLabPaperConfig: () => request('/index-strategy/oi-lab/paper/config'),
  oiLabPaperConfigSave: (config) => request('/index-strategy/oi-lab/paper/config', { method: 'POST', body: JSON.stringify({ config }) }),
  oiLabPaperPositions: (days = 30) => request(`/index-strategy/oi-lab/paper/positions?days=${days}`),
  oiLabPaperOpen: (signal_id, mode, lots) => request('/index-strategy/oi-lab/paper/open', { method: 'POST', body: JSON.stringify({ signal_id, mode, lots }) }),
  oiLabPaperClose: (id) => request(`/index-strategy/oi-lab/paper/${id}/close`, { method: 'POST' }),

  // ── Data Ingestion Lab (upload / check / explore Market Store history) ──
  diStage: (files) => { const fd = new FormData(); [...files].forEach((f) => fd.append('files', f)); return requestUpload('/data-ingestion/stage', fd); },
  diStages: () => request('/data-ingestion/stages'),
  diPreview: (id) => request(`/data-ingestion/stage/${id}`),
  diDropStage: (id) => request(`/data-ingestion/stage/${id}`, { method: 'DELETE' }),
  diValidate: (body) => request('/data-ingestion/validate', { method: 'POST', body: JSON.stringify(body) }),
  diCommit: (body) => request('/data-ingestion/commit', { method: 'POST', body: JSON.stringify(body) }),
  diJob: (id) => request(`/data-ingestion/job/${id}`),
  diCoverage: () => request('/data-ingestion/coverage'),
  diDeletePartition: (body) => request('/data-ingestion/partition/delete', { method: 'POST', body: JSON.stringify(body) }),
  diExploreSpot: (p) => request(`/data-ingestion/explore/spot?${new URLSearchParams(p)}`),
