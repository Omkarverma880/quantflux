...
import {
  RefreshCw, Loader2, AlertCircle, Save, FlaskConical, Download, Play, Info,
} from 'lucide-react';
import { api } from '../../api';
...
const COLS = [
  ...
  ['entry_ce', 'Entry CE'], ['exit_ce', 'Exit CE'], ['entry_pe', 'Entry PE'], ['exit_pe', 'Exit PE'],
    ...
    <div className="p-4 md:p-6 space-y-4 max-w-[1500px] mx-auto">
      ...
      {!data && !loading && (
        <div className="bg-surface-2 border border-surface-3 rounded-xl p-10 text-center text-gray-500 text-sm flex items-center justify-center gap-2">
          <Info className="w-4 h-4" /> Pick a stock (or All F&O), a date range, and click Run Backtest. Live-day append & the Summary Report are coming next.
        </div>
      )}
    </div>
  );
