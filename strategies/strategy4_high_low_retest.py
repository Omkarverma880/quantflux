...
class Strategy4HighLowRetest:
    ...
    def get_intraday_series(self, target_day: Optional[date] = None) -> list[dict]:
        ...
        until now (or full session for a back-date). Used to seed the
        live chart so it shows real intraday movement, not just ticks
        ...
        # Cap at 'now' for today, full session for past dates
        now = datetime.now()
        ...
        try:
            ...
            logger.warning("S4 intraday fetch failed: %s", exc)
            return []
        out: list[dict] = []
        for c in candles:
            ts = c.get("date")
                ...
                if str(o.get("order_id")) == str(self.entry_order["order_id"]):
                    ...
                    if status == "COMPLETE":
                        ...
                        if (
                            ...
                            logger.warning(
                                "S4 entry slippage breach: ref=%.2f fill=%.2f slip=%.2f > max=%.2f — flattening",
                        ...
                        logger.warning(
                            "S4 entry %s — resetting to IDLE for re-arm", status,
                        )
                        self.entry_order = None
                        self.signal_type = None
                        self.signal = "NO_TRADE"
    ...
    def _check_exit(self):
            ...
            for o in orders:
                ...
                if (
                    ...
                    if status in ("CANCELLED", "REJECTED"):
                        # Exchange rejected our SL leg — revert to shadow so
                        # the next tick re-attempts when LTP is in range
                        # instead of leaving the position unprotected.
                        logger.warning(
                            "S4 SL order %s on exchange (%s) — reverting to shadow",
                            oid, status,
                        )
                        self.sl_order = None
                        self.sl_shadow = True
                if (
                    ...
                    if status in ("CANCELLED", "REJECTED"):
                        logger.warning(
                            "S4 TGT order %s on exchange (%s) — reverting to shadow",
                            oid, status,
                        )
                        self.target_order = None
                        self.target_shadow = True
        ...
        if self.sl_shadow and ltp <= (self.sl_price + self.sl_proximity):
            logger.info(
                "S4 SL proximity hit: ltp=%.2f sl=%.2f prox=%.2f — promoting",
            ...
            try:
                if ltp <= self.sl_price:
                    # Already below SL — emit a MARKET sell to guarantee
                    # exit (LIMIT @ ltp*0.9 was sometimes left unfilled).
                    self.broker.place_order(OrderRequest(
                        ...
                        product=ProductType.MIS, tag="S4SL",
                    ))
                    logger.warning("S4 SL breach — MARKET sell @ ltp=%.2f", ltp)
                ...
                logger.info("S4 SL-M order placed on exchange: %s", resp.order_id)
                self._save_state()
            except Exception as exc:
        ...
        if self.target_shadow and ltp >= (self.target_price - self.target_proximity):
            logger.info(
                "S4 TGT proximity hit: ltp=%.2f tgt=%.2f prox=%.2f — promoting",
            ...
            try:
                if ltp >= self.target_price:
                    # Already past target — MARKET sell to guarantee exit.
                    self.broker.place_order(OrderRequest(
                        tradingsymbol=self.option_symbol,
                        exchange=Exchange.NFO, side=OrderSide.SELL,
                        quantity=self.quantity, order_type=OrderType.MARKET,
                        product=ProductType.MIS, tag="S4TGT",
                    ))
                    logger.warning("S4 TGT breach — MARKET sell @ ltp=%.2f", ltp)
                ...
                logger.info("S4 TGT LIMIT order placed on exchange: %s", resp.order_id)
                self._save_state()
            except Exception as exc:
    ...
    def _slippage_flatten(self, ref_ltp: float, slip: float):
        ...
        if not settings.PAPER_TRADE and self.option_symbol and self.lot_size > 0:
            try:
                ...
                logger.error("S4 slippage-flatten failed: %s", exc)
    ...
    def _run_backtest_sim(
        ...
        """Pure-python replay using spot-Δ≈1 proxy.

        Why a proxy: Zerodha's historical_data API does not reliably
        return minute-level option prices for past expiries (rate-limited,
        contract often dropped from instrument master after expiry). To
        keep backtest reproducible across any sim_day we replay using
        spot moves only and convert them to option PnL via Δ≈1.

        We still compute the EXACT ITM strike + tradingsymbol the live
        engine would have used, so the user sees which contract would
        have been bought.
        """
        ...
        def _record_trade(t_str: str, exit_spot: float, exit_type: str, label: str):
            # Δ≈1: option PnL = favorable spot move × lot
            if side == "CE":
        ...
        for c in candles:
            ...
            # Position management — SL/TGT in option points = spot points (Δ=1)
            if st == "POSITION_OPEN":
                if side == "CE":
                    ...
                    if adverse >= sl_pts:
                        ...
                        trades_done += 1
                        st = "COMPLETED"  # mirror live: SL stops day
                        side = None; continue
                    if favor >= tgt_pts:
        ...
        return {
            ...
            "note": (
                ...
                "Live engine buys real ITM CE/PE at the printed strike — "
                "actual option PnL will differ slightly due to theta/IV. "
